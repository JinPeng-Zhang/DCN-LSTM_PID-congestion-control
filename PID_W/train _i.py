...
grad = []
for j in range(20):
    file = open("WD/DATA_-0.6/{}.txt".format(j+2),"r")
    line  =file.readline()
    data = []
    num = 0
    for i in range(200):
    ...
    while line:
        ...
        if num<5000:
            data.append([float(oldrate.split(":")[-1]),float(oldrtt.split("t")[-1]),float(c.split(":")[-1]),float(eavg.split(":")[-1])])
            num = num+1
    ...
    for i in range(len(data)-14):
        grad.append(tra.grad(np.mean([t[1] for t in data[i+1:i+3]]),data[i][3],data[i][0],data[i][1],data[i][2]))
