...
grad = []
for j in range(20):
    file = open("W/1.045_0.1_0.3/{}.txt".format(j+2),"r")
    line  =file.readline()
    data = []
    num = 0
    for i in range(200):
    ...
    while line:
        ...
        if num<5000:
            data.append([float(oldrate.split(":")[-1]),float(oldrtt.split("t")[-1]),float(c.split(":")[-1]),float(e.split(":")[-1])])
            num = num+1
